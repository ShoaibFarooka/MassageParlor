...
const createUser = async (userData, role) => {
  ...
  if (existingUser) {
    const error = new Error(
      "A user with that number has already been registered!"
    );
    error.code = 409;
    throw error;
  }
...
const updateUser = async (userId, updateData, role = null) => {
  ...
  if (role && userToUpdate.role !== role) {
    const error = new Error(
      "Cannot update other type of user via this endpoint!"
    );
    error.code = 405;
    throw error;
  }
...
const deleteUser = async (userId, role) => {
  ...
  if (!userToDelete.role === role) {
    const error = new Error(
      "Cannot delete other type of user via this endpoint!"
    );
    error.code = 405;
    throw error;
  }
