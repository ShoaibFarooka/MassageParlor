...
function CreateEditService({ setSelectedImage, selectedImage, setUser, user, isOpen, onClose, selectedService, onLoad }) {
    ...
    const [firstName, ...rest] = (selectedService?.name || "").split(" ");
    const surname = rest.join(" ") || selectedService?.surname || "";
    ...
    useEffect(() => {
        if (selectedService) {
            ...
            setSelectedImage(selectedService?.image);
        }
    }, [selectedService, firstName, surname]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (selectedService) {
                response = await userService.updateUserProfile(formData, selectedService._id);
            } else {
                response = await userService.registerUser(formData, 'service-provider');
            }
            if (!response) {
    ...
    return (
        <CustomModal
            ...
            width="697px"
            contentLabel={selectedService ? "Edit Service Provider" : "Add Service Provider"}
        >
            <div>
                <form onSubmit={handleSubmit} autoComplete="off" className="mt-2 p-10">
                    <div className="flex justify-center">
                        <div className="relative w-24 h-24 rounded-full bg-gray-100 flex items-center justify-center">
                            {selectedImage || selectedService?.image ? (
                                <img
                                    src={
                                        selectedImage
                                            ? selectedImage
                                            : `http://localhost:5777/static/images/${selectedService.image}`
                                    }
                                    alt="Profile"
